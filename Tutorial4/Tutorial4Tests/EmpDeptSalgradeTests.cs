...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var chicagoDeptsNo = depts.Where(e => e.Loc == "CHICAGO").Select(e => e.DeptNo).ToList();

        List<Emp> result = emps.Where(e => chicagoDeptsNo.Contains(e.DeptNo)).ToList(); 
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, dept => dept.DeptNo,(e,dept) => new {e.EName, dept.DName}).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e => grades.Where(g => e.Sal >= g.Losal && e.Sal <= g.Hisal).Select(g => new { e.EName, g.Grade }))
            .ToList();
    }
